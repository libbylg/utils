...
//! `struct token_parser_error` defined a object to save to information of the parser error.
struct token_error_t {
    int  id;
    char error[500];
};
EXPORT_API int token_error_vprintf(struct token_error_t* error, const char* format, va_list va);
EXPORT_API int token_error_printf(struct token_error_t* error, const char* format, ...);


//!  `struct token_parser_reader_t` defined the interface to read more data from the data source.
struct token_reader_t {
    ...
    //! Return 0 will hit the end of the data, or return  -1 to to indicate the error.
